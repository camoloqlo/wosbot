...
public class WarAcademyTask extends DelayedTask {
    ...
    protected void execute() {
        // STEP 1: I need to go to left menu, then check if there's 2 matches of
        // research template
        // left menu
        ...
        for (int attempt = 1; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
            ...
            researchResults = emuManager.searchTemplates(EMULATOR_NUMBER,
                    EnumTemplates.GAME_HOME_SHORTCUTS_RESEARCH_CENTER, 90, 2);
            ...
            if (researchResults.size() >= 2) {
                ...
                logWarning(
                        "Only " + researchResults.size() + " research centers were found on attempt " + attempt + ".");
        ...
        if (researchResults.size() < 2) {
            logError("Not enough research centers were found after " + MAX_RETRY_ATTEMPTS
                    + " attempts. Stopping the task.");
            return;
        }
        ...
        DTOImageSearchResult researchButton = searchTemplateWithRetries(EnumTemplates.BUILDING_BUTTON_RESEARCH, 3);

        if (!researchButton.isFound()) {
            logError("The research button was not found after multiple attempts.");
        ...
        DTOImageSearchResult warAcademyUi = searchTemplateWithRetries(EnumTemplates.VALIDATION_WAR_ACADEMY_UI, 3);

        if (!warAcademyUi.isFound()) {
            logError("The War Academy UI was not found after multiple attempts.");
            reschedule(LocalDateTime.now().plusMinutes(5));
            return;
        }
        ...
        for (int ocrAttempt = 1; ocrAttempt <= MAX_RETRY_ATTEMPTS; ocrAttempt++) {
            ...
            try {
                ...
                if (parsed >= 0) {
                    ...
                    logWarning("OCR attempt " + ocrAttempt + " failed to find a numeric value in the result: "
                            + ocrResult);
        ...
        if (remainingShards == -1) {
            logError("OCR failed to find any numeric value after " + MAX_RETRY_ATTEMPTS
                    + " attempts. Rescheduling the task.");
            reschedule(LocalDateTime.now().plusMinutes(5));
            return;
        }
        ...
        for (int finalOcrAttempt = 1; finalOcrAttempt <= MAX_RETRY_ATTEMPTS; finalOcrAttempt++) {
            ...
            logDebug("Reading the final number of remaining shards via OCR (Attempt " + finalOcrAttempt + "/"
                    + MAX_RETRY_ATTEMPTS + ")...");
            ...
            try {
                ...
                if (parsed >= 0) {
                    ...
                    logInfo("Final OCR was successful on attempt " + finalOcrAttempt + ". Found " + finalRemainingShards
                            + " shards.");
                    break;
                } else {
                    logWarning("Final OCR attempt " + finalOcrAttempt
                            + " failed to find a numeric value in the result: " + ocrResult);
        ...
        if (finalRemainingShards > 0) {
            logInfo("Additional shards were found: " + finalRemainingShards
                    + ". Rescheduling the task to redeem them.");
